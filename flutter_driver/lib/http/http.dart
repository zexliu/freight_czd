...
import 'package:flutterdriver/pages/login.dart';
...
class HttpManagerConfig {
 static String baseUrl = "https://www.czdhy.com"; //home
 static String wsBaseUrl = "ws://39.107.110.237:8088";
 //  static String baseUrl = "http://api.zex.nat300.top";
 //  static String wsBaseUrl = "ws://192.168.1.4:8088";
}
...
class HttpManager {
  ...
  static String clientId = "driver_client";
    ...
    if (null == _dio) {
      ...
      _dio.interceptors.add(InterceptorsWrapper(
        ...
        onError: (DioError err) async {
          ...
          if (err.response.statusCode == 401&& err.request.path != "/oauth/token") {
            String refreshToken =
...
//    dio.interceptors.add(LogInterceptor());
    return await dio.post("/oauth/token", queryParameters: {
      "grant_type": "refresh_token",
      "refresh_token": refreshToken
    },options: RequestOptions(
      headers: {"Authorization": "Basic "  + _base64Encode('$clientId:$secret')}
    )).then((value) {
      ...
      _dio.unlock();
      Global.navigatorKey.currentState.pushAndRemoveUntil(MaterialPageRoute(builder: (BuildContext context) {
        return new LoginPage();
      }), (route) => false);
    });
  }
