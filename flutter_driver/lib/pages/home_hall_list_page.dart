...
class HomeHallListPage extends StatefulWidget {
  final OftenLineRecord item;

  const HomeHallListPage({Key key, this.item}) : super(key: key);

  @override
  _HomeHallListPageState createState() => _HomeHallListPageState();
}

class _HomeHallListPageState extends State<HomeHallListPage> {
  GlobalKey _stackKey = GlobalKey();
  DropdownMenuController _dropdownMenuController = DropdownMenuController();
  List<String> _dropDownHeaderItemStrings = ['时间排序', "筛选"];
  ...
  void _onRefresh() async {
    ...
    HttpManager.getInstance().get("/api/v1/deliver/goods", params: {
      "loadAreas": widget.item.loadAreas,
      "unloadAreas": widget.item.unloadAreas,
      "status": 1,
      "current": _current,
      "size": _size,
...
//    _customDates.add(CustomDate("距离排序", "distance"));
    ...
    if (widget.item.carLongs != null) {
      this._currentCarLongs = widget.item.carLongs.split(",");
    }
    if (widget.item.carModels != null) {
      this._currentCarModels = widget.item.carModels.split(",");
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          bottom: AreasLayout(
              text:
                  "${_getAddress(widget.item.loadAreas)} → ${_getAddress(widget.item.unloadAreas)}"),
        ),
...
class AreasLayout extends StatelessWidget implements PreferredSizeWidget {
  final String text;

  const AreasLayout({Key key, this.text}) : super(key: key);
