...
import 'package:flutterdriver/plugin/baidu_face_plugin.dart';
...
class _AuthenticationPageState extends State<AuthenticationPage> {
  ...
  String currentCarLong;
  String currentCarModel;
  String carModel;
  IdentityResult identity;
  DictEntryRecord selectedNature;
  DictEntryRecord selectedCarLong;
  DictEntryRecord selectedCarModel;



  @override
  void initState() {
    fetchDict("driver_nature").then((value) => this.deliverNatures = value.records);
                              ...
                              children: <Widget>[
                                ...
                                Text(
                                  this.selectedCarModel == null
                                      ? "请选择车型"
                                      : this.selectedCarModel.dictEntryName,
                                  style: TextStyle(color: Colors.grey),
                                )
                              ],
                            ),
                          ...
                          children: <Widget>[
                            Row(
                              children: <Widget>[
                                ...
                                Text(
                                  this.selectedCarLong == null
                                      ? "请选择车长"
                                      : this.selectedCarLong.dictEntryName,
                                  style: TextStyle(color: Colors.grey),
                                )
                              ],
                            ),
  ...
  DictEntryRecord _getModel() {
    if (this.currentCarModel == null) {
      return null;
    }
    for (var value in this.carModels) {
      if (value.dictEntryValue == this.currentCarModel) {
        return value;
      }
    }
    return null;
  }
  DictEntryRecord _getLong() {
    if (this.currentCarLong == null) {
      return null;
    }
    for (var value in this.carLongs) {
      if (value.dictEntryValue == this.currentCarLong) {
        return value;
      }
    }
    return null;
  ...
  _onSubmitPressed() {
    ...
    HttpManager.getInstance().post("/api/v1/authenticate/driver", data: {
      ...
      "nature": selectedNature.dictEntryName,
      "carLong": selectedCarLong.dictEntryName,
      "carModel": selectedCarModel.dictEntryName,
    }).then((value) {
