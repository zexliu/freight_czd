...
import 'package:freightowner/pages/evaluation_submit_page.dart';
import 'package:freightowner/pages/order_details.dart';
...
//                                    ),
                                      InkWell(
                                        ...
                                        onTap: () {
                                          PhoneUtils.call(item.driverMobile,
...
//  )
  ...
  _buildActions(BuildContext context, OrderRecord item) {
    ...
    if (item.driverPayStatus && !item.refundStatus) {
      widgets.add(InkWell(
        child: Text(
          "退还订金",
          style: TextStyle(fontSize: 12),
        ...
        onTap: () async {
          final result =
              await Navigator.of(context).push(MaterialPageRoute(builder: (c) {
            return OrderRefundPage(item.orderId, item.amount);
          }));
          if (result == "success") {
            _refreshController.requestRefresh();
          } else {
    ...
    if (!item.confirmStatus) {
      widgets.add(InkWell(
        ...
        onTap: () {
          showLoadingDialog();
          HttpManager.getInstance()
              .post("/api/v1/orders/confirm/${item.orderId}")
      ...
      widgets.add(InkWell(
        child: Text(
          item.protocolStatus == 1 || item.protocolStatus == 2
              ? "查看协议"
              : "签订协议",
          style: TextStyle(fontSize: 12),
        ),
